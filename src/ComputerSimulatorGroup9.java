...
public class ComputerSimulatorGroup9 extends JFrame {
    ...
    static Set<String> Xop=new HashSet<>();
    ...
    private int FR0;    //Stores FR0 value
    private int FR1;    //Stores FR1 value
    ...
    public int InputSignal; //Stores Input Signal status
    public int InputVal; //Stores Input Value
    /**
     * Creates new form ComputerSimulatorGroup9
     */
    ...
    static JFrame f;
    static JButton Store, Load, Clear_All, SS, Run, Init,Assembler, Custom_Init;
    ...
    static {
        opMap.put("TRAP", "011110");
        opMap.put("LDR", "000001");
        opMap.put("STR", "000010");
        opMap.put("LDA", "000011");
        opMap.put("LDX", "101001");
        opMap.put("STX", "101010");
        opMap.put("JZ", "001010");
        opMap.put("JNE", "001011");
        opMap.put("JCC", "001100");
        opMap.put("JMA", "001101");
        opMap.put("JSR", "001110");
        opMap.put("RFS", "001111");
        opMap.put("JGE", "010001");
        opMap.put("AMR", "000100");
        opMap.put("SMR", "000101");
        opMap.put("AIR", "000110");
        opMap.put("SIR", "000111");
        opMap.put("MLT", "010100");
        opMap.put("DVD", "010101");
        opMap.put("TRR", "010110");
        opMap.put("AND", "010111");
        opMap.put("ORR", "011000");
        opMap.put("NOT", "011001");
        opMap.put("SRC", "011111");
        opMap.put("RRC", "100000");
        opMap.put("IN", "111101");
        opMap.put("OUT", "111110");
        opMap.put("CHK", "111111");
        opMap.put("FADD", "100001");
        opMap.put("FSUB", "100010");
        opMap.put("VADD", "100011");
        opMap.put("VSUB", "100100");
        opMap.put("CNVRT", "100101");
        opMap.put("LDFR", "110010");
        opMap.put("STFR", "110011");
    }
...
private String Address(String num){
        String add=Integer.toBinaryString(Integer.parseInt(num));
    if(add.length()==1){
        add="0000"+add;
    }
    ...
    private String OpCode(String num,int length){
        String reg=Integer.toBinaryString(Integer.parseInt(num));
        if(length==2&&reg.length()<2){
            reg="0"+reg;
        } else if (length==3&&reg.length()<3) {
            reg="00"+reg;
        }
        return reg;
    }
    private String AssemblerEncode(String[] content) throws InterruptedException {
        String Op=content[0];
        if(Arrays.toString(content).contains("HLT")||Arrays.toString(content).contains("hlt")){
            IR=0;
            ExecuteInstruction();
            return "0000000000000000";
        }
        String[] decode=content[1].split(",");

        String valu="";
        if(Op.equalsIgnoreCase("Data")){
            valu="000000";
        }
        ...
        try {
            if (decode.length == 1) {
                if(Op.equalsIgnoreCase("Data")){
                    valu +="00000" + Address(decode[0]);
                }
                else{
                    valu+=Op.equalsIgnoreCase("RFS")?"00000"+OpCode(decode[0],2) :
                            OpCode(decode[0],2)+"00000000";
//RFS and NOT
                }
            } else if (decode.length == 2) {
                if(ioOps.contains(Op)) {
                    valu += OpCode(decode[0],2) + "000"
                            + Address(decode[1]);
                    //IN OUT CHK
                }
                else if(arithmeticOps.contains(Op)){
                    valu += OpCode(decode[0],2)+OpCode(decode[1],2) +"000000";
                    //MLT DVD TRR AND ORR
                }
                else if(Xop.contains(Op)){
                    valu +="00"+ OpCode(decode[0],2)+"0"
                            + Address(decode[1]);
                    //LDX STRX JMA JSR
            ...
            if(Op.equalsIgnoreCase("data")) {
                System.out.println(content[1]+" "+loc+"lop "+Integer.toBinaryString(loc));
                opcodeMap.put(content[1], Integer.toString(loc));
                System.out.println(opcodeMap);
                symbolMap.put(content[1], "0");
                System.out.println(symbolMap + "hi");
                valu = Integer.toBinaryString(PC);
            }
            }
...
System.out.println(valu+"value");
    ...
    private String[] writeLine(int value, int location){
        String result = Integer.toHexString(value);
        String locRes = Integer.toString(location, 16);
        //String locRes= Integer.toHexString(location);
        ...
        else if(locRes.length()==2){
            locRes="00"+locRes;
        }
        else if(locRes.length()==1){
            locRes="000"+locRes;
        }
        String[] res=new String[]{locRes,result};
        return res;
    }
    ...
    private void AssemblerActionPerformed(java.awt.event.ActionEvent evt) {//Reads the init.txt file and loads the pc with the first address in the file
        dict= new Hashtable<>();
        loc=0;
        ...
        try {
            ...
            while ((st = br.readLine()) != null) {
                ...
                if (content[0].contains(":")) {
                    String label = content[0].substring(0, content[0].length() - 1);
                    System.out.println(loc+"123");
                    for(Map.Entry<String, String> me : symbolMap.entrySet()) {
                        if (symbolMap.containsKey(me.getKey())) {
                            symbolMap.replace(label,me.getValue(),String.valueOf(loc));
                        }
                    }
                    System.out.println(loc+"1234");
                    String stri=content.length>2?content[1]+" "+content[2]:content[1];
                    System.out.println(symbolMap);
                    System.out.println("iop "+stri);
                    content=stri.split(" ");
                }
                if(count==1 && content[0].equalsIgnoreCase("LOC")) {
                    loc = Integer.parseInt(content[1]);
                }
                else if(count!=1&&content[0].equalsIgnoreCase("LOC")){
                    loc=Integer.parseInt(content[1]);
                }
                else{
                    if(count==1){
                        System.out.println("yes loc"+loc);
                        loc = 0;
                    }
                    System.out.println(loc+"oplpo");
                    int location = Integer.parseInt(String.valueOf(loc));
                    int value = Integer.parseInt(AssemblerEncode(content),2);
                    System.out.println(location+"klo"+value);
                    opcodeMap.put(String.valueOf(location),String.valueOf(value));
                    System.out.println(opcodeMap);
                    loc++;
                    System.out.println(location + " " + value);
                }}
            ...
            for (Map.Entry<String, String> me : symbolMap.entrySet()) {
                if (opcodeMap.containsKey(me.getKey())) {
                    String addr = opcodeMap.get(me.getKey());
                    opcodeMap.remove(me.getKey());
                    opcodeMap.put(addr, me.getValue());
                    System.out.println(opcodeMap);
                    System.out.println(addr+"po"+me.getValue());
                } else {
            ...
            try {
                ...
                List<String> keysList = new ArrayList<>(opcodeMap.keySet());

                List<Integer> keysIntList = new ArrayList<>();

                for (String key:
                        keysList) {
                    keysIntList.add(Integer.parseInt(key));
                }
                Collections.sort(keysIntList);
                for (int key : keysIntList) {
                    int value = Integer.parseInt(opcodeMap.get(String.valueOf(key)));
                    String[] result = writeLine(value, key);
                    String newline = result[0] + " " + result[1];
    ...
    public int
    getAddress(int x) { //Returns the Address of x
    ...
    public boolean memoryOutofBoundCheck(int memoryAdr) { //Checks the memory size
        if (memoryAdr > 2056) {
            MFR = 8;
            PC = (int) (memory[1]);
            Populate_PC();
        ...
        if (IR_binary.length() <= 10) {
            ...
            if (IR_binary.length() == 11 && IR_binary.charAt(0) == '1') {
                ...
                if (flag_I == 0) {
                    ...
                    } else  //Indexed
                    {
                        switch (flag_IXR) {
                            case 1 -> {
                                ...
                                MAR = (int) (getAddress(IR) + X1);
                                if (memoryOutofBoundCheck(MAR)) return;
                            }
                            case 2 -> {
                                System.out.println("IX2 I=0 ");
                                MAR = (int) (getAddress(IR) + X2);
                                if (memoryOutofBoundCheck(MAR)) return;
                            }
                            case 3 -> {
                                System.out.println("IX3 I=0 ");
                                MAR = (int) (getAddress(IR) + X3);
                                if (memoryOutofBoundCheck(MAR)) return;
                            }
                            default -> {
                            }
                    ...
                    System.out.println(flag_GPR+"GPR");
                    switch (flag_GPR) {
                        case 1 -> {
